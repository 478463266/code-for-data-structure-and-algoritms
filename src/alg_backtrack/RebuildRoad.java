...
/*
ref:
1.https://my.oschina.net/floor/blog/843961
2.https://github.com/floor07/DataStructuresAndAlgorithm-Demo/blob/master/src/main/java/chapter10/recollection/RebulidRoad.java
 */
public class RebuildRoad {
    ...
    private static boolean place(int[] x, PriorityQueue<Integer> distSet, int n, int left, int right) {
        ...
        if (!found && CheckIfleft(x, distSet, n, left, right, dmax)){
            x[left] = x[n] - dmax;
            for (int j = 1; j < left; j++)
                distSet.remove(Math.abs(x[n] - dmax - x[j]));
            for (int j = right + 1; j<= n; j++)
                distSet.remove(Math.abs(x[n] - dmax - x[j]));
            found = place(x, distSet, n, left + 1, right);
            if (!found){
                for (int j = 1; j < left; j++)
                    distSet.add(Math.abs(x[n] - dmax - x[j]));
                for (int j = right + 1; j <= n; j++)
                    distSet.add(Math.abs(x[n] - dmax - x[j]));
            }
        }
    ...
    private static boolean CheckIfleft(int[] x, PriorityQueue<Integer> distSet, int n, int left, int right, int dmax) {
        ...
        for (int i = 1; i < left; i++) {
            flagL = flagL && distSet.contains(Math.abs(x[n] - x[i] - dmax));
            if (!flagL)
                break;
        }
        ...
        if (flagL){
            for (int i = 1; i < left; i++) {
                distSet.remove(Math.abs(x[n] - dmax - x[i]));
            }
            ...
            for (int i = 1; i < left; i++)
                distSet.add(Math.abs(x[n] - x[i] -dmax));
        }
    ...
    private static boolean CheckIfRight(int[] x, PriorityQueue<Integer> distSet, int n, int left, int right, int dmax) {
        ...
        for (int i = 1; i < left; i++){
            flagR = flagR && distSet.contains(Math.abs(x[i] - dmax));
            if (!flagR)
                break;
        }
        ...
        if (flagR){
            for (int i = 1; i < left; i++){
                distSet.remove(Math.abs(x[i] - dmax));
            }
            for (int i = right + 1; i <= n; i++){
                flagR = flagR && distSet.contains(Math.abs(x[i] - dmax));
                if (!flagR)
    ...
    public static void main(String[] args) {
        int[] x = new int[7];
        PriorityQueue<Integer> distSet = new PriorityQueue<>(15, new Comparator<Integer>() {
            @Override
